...
class StanfordTagger(TaggerI):
    ...
    def __init__(self, path_to_model, path_to_jar=None, encoding='utf8', verbose=False, java_options='-mx1000m', base_path='', appendix_jar_path=''):
        ...
        self._stanford_model = find_file(os.path.join(base_path, path_to_model),
    ...
    def parse_output(self, text, sentences = None):
        ...
        for tagged_sentence in text.strip().split("\n"):
            ...
            for tagged_word in tagged_sentence.strip().split():
                idx_separator = tagged_word.rfind(self._SEPARATOR)
                sentence.append((''.join(tagged_word[:idx_separator]), tagged_word[idx_separator + 1:]))
            tagged_sentences.append(sentence)
        return tagged_sentences, text
    ...
    def parse_output(self, text, sentences):
        if self._FORMAT == 'slashTags':
            ...
            for tagged_sentence in text.strip().split("\n"):
                for tagged_word in tagged_sentence.strip().split():
                    idx_separator = tagged_word.rfind(self._SEPARATOR)
                    tagged_sentences.append((''.join(tagged_word[:idx_separator]), tagged_word[idx_separator + 1:]))
                
            # Separate it according to the input
            raw_result = []
            texts = text.split(r' ')
            texts = sum(map(lambda x: x.split(r'\n'), texts), [])
            result = []
            start = 0 
            for sent in sentences:
                result.append(tagged_sentences[start:start + len(sent)])
                raw_result.append(texts[start:start + len(sent)])
                raw_result.append(['\n'])
                start += len(sent)

            raw_result = ' '.join(sum(raw_result, []))
